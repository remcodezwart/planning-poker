...
class UserModel
{
    ...
    public static function getPublicProfilesOfAllUsers()
    {
        ...
        $sql = "SELECT user_id, user_name, user_email, user_active, user_deleted FROM users";
    ...
    public static function getChambers()
    {
         $database = DatabaseFactory::getFactory()->getConnection();

         $query = $database->prepare("SELECT chambers.active,users.user_id ,users.user_name, chambers.name,chambers.subject,chambers.user_id,chambers.id AS chamberid ,chambers.eens FROM users INNER JOIN chambers ON chambers.user_id=users.user_id AND chambers.active=1");
         $query->execute();
    ...
    public static function deleteUsers($user_name)
    {
        $id = self::getUserIdByUsername($user_name);
      
        if ($id == null) {
            Session::add('feedback_negative', Text::get('FEEDBACK_USER_DELETE_NO_ID'));
            return false;
            exit();
        }
        $database = DatabaseFactory::getFactory()->getConnection();
        $query = $database->prepare("UPDATE users SET  user_deleted=:userdelete  WHERE user_id=:id");
        $query->execute(array(':id' => $id, ':userdelete' => '1'));
